...
[![Testing Report](https://github.com/SaaShup/netbox-gcore-plugin/actions/workflows/main_ci.yml/badge.svg)](https://github.com/SaaShup/netbox-gcore-plugin/actions/workflows/main_ci.yml)

Manage GCore with Netbox & style.
...
pip install netbox-gcore-plugin
```
...
```bash
cd $PROJECT
git clone git@github.com:SaaShup/netbox-gcore-plugin.git
...
Install netbox-gcore-plugin dependencies:

```bash
cd $PROJECT/netbox-gcore-plugin
pip install -e .
```
...
```bash
cd $PROJECT/netbox
cp $PROJECT/netbox-gcore-plugin/netbox_configuration/configuration_dev.py $PROJECT/netbox/netbox/netbox/configuration.py
...
python3 netbox/manage.py test netbox_gcore_plugin.tests --keepdb -v 2
```
...
```bash
cd $PROJECT/netbox
coverage run --include='*/netbox_gcore_plugin/*' netbox/manage.py test netbox_gcore_plugin.tests --keepdb -v 2
coverage report -m
```
